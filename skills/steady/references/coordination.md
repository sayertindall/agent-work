...
Give each worker the outcome, exact repository and baseline, owned paths, allowed actions, acceptance criteria, and a return format: artifact locations, observed checks, unresolved concerns. Tell workers they are not alone and must preserve others' changes. Concurrent writers get separate worktrees or clones and unique outputs. Isolate runtime state too when sockets, ports, databases, or caches could collide.
